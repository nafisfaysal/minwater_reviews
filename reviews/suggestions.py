...
def update_clusters():
    ...
    if num_reviews % update_step == 0:
        ...
        all_mineralwater_ids = set(map(lambda x: x.mineralwater.id, Review.objects.only("mineralwater")))
        num_users = len(all_user_names)
        ratings_m = dok_matrix((num_users, max(all_mineralwater_ids) + 1), dtype=np.float32)
        for i in range(num_users):
            user_reviews = Review.objects.filter(user_name=all_user_names[i])
            for user_review in user_reviews:
                ratings_m[i, user_review.mineralwater.id] = user_review.rating
